...
while True:
    ...
    for encodeFace, box in zip(encodes, boxes):
        ...
        if matches[idx]:
            ...
            if name == "KIAN":
                ...
                uart.write(b'L1\n')
                fan_on = True
                fan_end_time = now + 120  # 2 minutes
                current_person = "KIAN"
                print("KIAN: Fan + Lights ON")

            elif name == "DYLAN":
                uart.write(b'L1\n')
                fan_on = True
                fan_end_time = now + 30
                current_person = "DYLAN"
                print("DYLAN: Lights ON")
    ...
    if fan_on and now > fan_end_time:
        if current_person == "KIAN":
            uart.write(b'F0\n')
            uart.write(b'L0\n')
            print("KIAN: Fan + Lights OFF")
        elif current_person == "DYLAN":
            uart.write(b'L0\n')
            print("DYLAN: Lights OFF")
        fan_on = False
        current_person = None
